...
| [pdf2md-quality-performance-optimization](plans/pdf2md-quality-performance-optimization.md) | 实施中 | 阶段 2 | 2026-07-24 | `coverage-validation-optimization`、`pdf-evaluation-suite`、`single-page-segmentation-migration`、`table-text-omission-detection`、`pdf2md-fix-manual-workflow`、`pdf-table-audit`、`pdf-table-repair`、`pdf-extract-data-table-coverage` | 阶段 1 已独立验收完成并关闭；阶段 2 Step 0 和独立准入通过，正在实施用户确认的 VLM sidecar 保守边界：视觉页/指定页、单批最多 10 页、失败 review、不覆盖主输出 | [阶段 2 当前阶段](plans/pdf2md-quality-performance-optimization.md#当前阶段) |
