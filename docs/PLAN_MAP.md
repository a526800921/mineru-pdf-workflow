...
| [llm-human-collaboration-migration](plans/llm-human-collaboration-migration.md) | 实施中 | 阶段 0：协作契约与基线冻结 | 2026-07-14 | `pdf2md-fix-manual-workflow`、`pdf-table-repair`、`pdf-extract-data-table-coverage`、`data-ingestion-pipeline`、`cli-only-migration`、ADR 0003 | 已达到阶段 0 实施准入；已完成真实包/临时副本基线复核并冻结用户确认、LLM 交付摘要格式；阶段 0 尚未完成，尚未实施 skill 合并或动态脚本 harness | [阶段准入摘要](plans/llm-human-collaboration-migration.md#阶段准入摘要) |
