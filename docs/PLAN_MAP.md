...
| [llm-human-collaboration-migration](plans/llm-human-collaboration-migration.md) | 实施中 | 阶段 3：`pdf2md-fix` 兼容迁移 | 2026-07-15 | `pdf2md-fix-manual-workflow`、`pdf-table-repair`、`pdf-extract-data-table-coverage`、`data-ingestion-pipeline`、`cli-only-migration`、ADR 0003 | 阶段 1、阶段 2 已独立验收关闭；阶段 3 兼容 skill 已完成收敛，独立验收尚未进行；阶段 4 尚未启动 | [阶段 3 实施证据](plans/llm-human-collaboration-migration.md#阶段-3-实施证据2026-07-15) |
...
| LLM/人工协作入口仍分为 `pdf2md` 与 `pdf2md-fix` | 按 `llm-human-collaboration-migration` 阶段 3 将 `pdf2md-fix` 收敛为兼容跳转；阶段 4/5 真实验收后再决定兼容入口最终状态 | skill 文档、用户协作方式、pdf2md-fix 兼容入口 | 否（阶段 3 已进入实施） | 实施中 |
