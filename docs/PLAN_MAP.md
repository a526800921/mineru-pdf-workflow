...
| [llm-human-collaboration-migration](plans/llm-human-collaboration-migration.md) | 实施中 | 阶段 1：统一 `pdf2md` 编排入口 | 2026-07-14 | `pdf2md-fix-manual-workflow`、`pdf-table-repair`、`pdf-extract-data-table-coverage`、`data-ingestion-pipeline`、`cli-only-migration`、ADR 0003 | 阶段 0 已完成；阶段 1 Step 0 已通过，`pdf2md` 主入口已增加人工协作编排章节并与用户级 skill 同步；`pdf2md-fix` 保留完整内容，阶段 3 再处理兼容跳转 | [阶段准入摘要](plans/llm-human-collaboration-migration.md#阶段准入摘要) |
