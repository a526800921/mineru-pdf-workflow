...
| [pdf-output-package-layout](plans/pdf-output-package-layout.md) | 已完成 | 阶段 8 输出包 + venv 环境兼容 + demo5 验收 | automated-pdf-pipeline、coverage-validation-optimization | [验收记录](plans/pdf-output-package-layout.md#验收记录2026-06-30) |
...
| 输出产物分散在旧目录 | 阶段 8 已完成：统一为 `<package>/` 输出包结构，demo5 全路径验收通过 | `pdf-seg`、`pdf-merge`、`pdf-auto`、MCP 返回路径 | 否 | 已解决 |
