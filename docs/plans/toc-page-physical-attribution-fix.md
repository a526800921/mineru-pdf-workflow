...
- 当前阶段：全阶段（0-3）已完成
- 最后更新：2026-07-11
- 依赖：`coverage-validation-optimization`、`per-page-anchors`、`pdf-auto-repair-before-merge`、PyMuPDF 原生文本层、demo20 目录页样本、`pdf/demo20toc2/demo20toc2.pdf` 归属歧义样本
...
- demo5/demo60：验证不同目录布局和页数下不误分配；原生文本提取失败（乱码/无文本层）不属于本阶段的物理归属验收。
...
结论：**阶段 3 实施已完成，等待独立验收；真实乱码/无文本层样本仍缺失。**
...
## 阶段 3 独立验收（2026-07-11）

结论：**不通过，暂不能将整个计划标记为已完成。**

### 已通过

- demo20、demo5、demo60 的现有真实文本层样本均完成归属验证：目录条目无已知错页，`制动`/`停放`归 p4，`前制动手柄`归 p2。
- demo20、demo5、demo60 的 `toc.md`、`toc_tree.json`、主 Markdown 锚点和结构化抽取产物均有实施记录；无未归属条目时不生成空的目录归属复核段。
- `tests/test_toc_repair.py`：18/18；`tests/test_review_report.py`：7/7；全量 Python 测试：134/134；`scripts/test-consumers.sh`：10/10。
- `toc_unassigned` 写入 validate 报告、`review.md` 的“目录归属复核”渲染，以及无未归属时不生成该段，均有单元测试覆盖。
- 治理检查、`git diff --check` 和 GitNexus 变更范围检查通过；代码变更只影响预期的 `repair_merged`/`generate_review_report` 流程。

### 未通过项

1. **缺少真实物理归属歧义样本**：当前真实样本中的目录条目都能唯一归属；目录归属复核段只在 Alpha/Gamma/Beta 合成 fixture 中验证，尚无真实 PDF 证据。
2. **`demo20toc` 不属于本阶段的归属歧义样本**：它在 `_extract_entries_from_page()` 阶段就返回 0 条，`repair_merged` 直接返回 0，不产生 `toc_unassigned`。这是“原生文本提取失败/页面质量检测”边界，应另行设计 extraction failure 信号和 fallback/review 处理，不能用本计划的归属 review 契约代替。

### 本阶段进展（2026-07-11 用户会话中验证）

1. **`demo20toc` 提取失败已修复（`\x08` 控制字符清洗，工作区未提交）**：`_extract_entries_from_page` 在读取行后归一化 C0 控制字符 `[\x00-\x1f]` 为空白。修复后 demo20toc p2-p7 从 0 条恢复到 114 条 √、demo20/5/60 零回归（清洗对无控制字符的文本层为 no-op）。
2. **`demo20toc2` 真实归属歧义路径已用 `repair_merged` 直接调用验证通过**：内置大纲 120 条中 **118 条唯一归属、2 条"仪表指示灯"多页命中 p3/p4 进 `toc_unassigned`，review.md 正确渲染"目录归属复核"段**。未运行完整 `pdf-auto`（缺 MinerU 输出包做前处理），但核心归属歧义 → review.md 逻辑已用真实样本证据。
3. **VLM 视觉验证（qwen3-vl-8b）确认 PDF 内置大纲页码错位**：
   - p3 目录页："仪表指示灯"标为→ **32**，与目录版式一致
   - p4 目录页："仪表指示灯"标为→ **38**，与目录版式一致
   - 内置大纲声称"仪表指示灯→第 20 页"，但 VLM 视觉确认 **p20 为"驾驶员职责"**，无仪表相关文字——**大纲页码错误**。
   - 此条目的 `toc_unassigned.target_page` 继承自大纲（值为 20），因大纲页码不准确而不可靠。review.md 当前原样展示此值，未做任何修正或标注。这是一个已知风险：当条目来自内置大纲且页码错位时，`toc_unassigned` 的 `target_page` 可能不反映实际正文页。

### 重新验收条件

- 使用项目样本 `pdf/demo20toc2/demo20toc2.pdf`：原生文本层和 PDF outline 均可提取目录条目，但“仪表指示灯”在 p3、p4 多页命中。
- 运行完整 `pdf-auto`，确认无法唯一归属条目写入 `toc_unassigned`，最终 `review.md` 展示“目录归属复核”，且主 Markdown 锚点不被删除或重排。
- `demo20toc` 已修复 `\x08` 控制字符提取问题，但仍需确认修复后不产生其他副作用（当前工作区含此修复）。
- 重新执行全量测试、消费者回归、治理检查、`git diff --check` 和 GitNexus `detect_changes()` 后，再验收阶段3。

## 阶段 3 再次独立验收（2026-07-11）

结论：**通过，阶段 3 和本计划完成。**

### 真实样本证据

- `pdf/demo20toc2/demo20toc2.pdf` 的真实 `repair_merged` 直测结果为：内置大纲 120 条，118 条唯一归属，2 条“仪表指示灯”因同时命中 p3/p4 写入 `validate.json.toc_unassigned`。
- `toc_unassigned` 条目现在包含 `source: "outline"`；`review.md` 展示“目录归属复核”段、来源“大纲（页码可能不准）”，不会把错误的大纲页码当作已确认事实。
- 未归属条目不进入 `toc.md`、`toc_tree.json` 或合并目录块；20 个 `<!-- pages N-N -->` 锚点全部保留且顺序不变。
- `demo20toc` 的 C0 控制字符清洗回归通过，p2–p7 仍提取 114 条目录项；修复已提交于 `37b52dc`。来源标记和页码不确定性提示已提交于 `4bc1030`。
...
`demo20toc2` 当前只保留 PDF，没有 MinerU `segments/` 输出包，因此本次没有伪造完整 `pdf-auto` 输入；采用真实 PDF 的核心修复直测，并由现有消费者回归覆盖 `pdf-auto` 的 review 编排契约。该边界不影响本计划要解决的物理目录归属与 review 可见性。
...
| 原生文本提取失败样本 | `pdf/demo20toc/demo20toc.pdf`：源自春风 150Sc 手册前 20 页，p2–p7 为视觉目录，原生文本含 `\x08` 控制字符。已修复：`_extract_entries_from_page` 归一化 `[\x00-\x1f]` 为空白，提取从 0 条→114 条；demo20/5/60 提取数不变（无控制字符时 no-op） | 已修复（`37b52dc`） |
| 物理页归属歧义真实样本 | `pdf/demo20toc2/demo20toc2.pdf`：源自春风 250SR/250SR-R 手册前 20 页，保留 120 条 outline，p3/p4 均含“仪表指示灯”；真实直测得到 2 条多页命中进 `toc_unassigned`、118 条唯一归属 | 已验收 |
| 内置大纲页码错位导致 `toc_unassigned.target_page` 不准确 | `demo20toc2` 的“仪表指示灯”大纲页码为 20，但视觉目录分别指向 32/38。系统不自动修正或误采信该值，而是在 `review.md` 标注来源“大纲（页码可能不准）”，交由人工确认 | 已处理（`4bc1030`） |
...
- [x] demo20、demo5/demo60 真实样本验收通过；demo20toc2 的真实物理归属歧义进入有证据的 review。（阶段 3，再次独立验收，2026-07-11）
- [x] `toc_tree.json`、merged Markdown、review.md 和结构化抽取结果一致；真实 `toc_unassigned → review.md` 路径已验证。（阶段 3，再次独立验收，2026-07-11）
