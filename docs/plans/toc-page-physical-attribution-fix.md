...
- 状态：实施中
- 当前阶段：阶段 3：真实样本与回归验收（独立验收未通过，待补边界样本）
- 最后更新：2026-07-11
...
| 原生文本提取失败样本 | `pdf/demo20toc/demo20toc.pdf`：源自春风 150Sc 手册前 20 页，p2–p7 为视觉目录，原生文本含 `\x08` 控制字符。已修复：`_extract_entries_from_page` 归一化 `[\x00-\x1f]` 为空白，提取从 0 条→114 条（含 `\x08` 清洗，工作区修复未提交）。demo20/5/60 提取数不变（无控制字符时 no-op） | 已修复，未提交 |
| 物理页归属歧义真实样本 | `pdf/demo20toc2/demo20toc2.pdf`：源自春风 250SR/250SR-R 手册前 20 页，保留 120 条 outline，p3/p4 均含“仪表指示灯”；当前规则 2 条多页命中、118 条归属，用于验证 `toc_unassigned → review.md`。超出截取范围的 outline 指向页已钳制到 p20，仅用于保持样本书签有效 | 待验收 |
| 内置大纲页码错位导致 `toc_unassigned.target_page` 不准确 | `demo20toc2` 的"仪表指示灯"条目：p3 目录表明→32，p4 目录表明→38，但内置大纲声称→20（VLM 确认 p20 为"驾驶员职责"，大纲错误）。`toc_unassigned` 继承大纲的 page=20，不反映实际指向。当前 review.md 原样展示该值，未做标注或修正；需评估是否对内置大纲来源的未归属条目标注"来自大纲，页码可能不准确" | 待评估 |
...
- [ ] demo20、demo5/demo60 真实样本验收通过或进入有证据的 review。（不同布局样本通过；物理归属歧义真实样本待补）
- [ ] `toc_tree.json`、merged Markdown、review.md 和结构化抽取结果一致。（常规路径通过；真实 `toc_unassigned → review.md` 路径待补）
