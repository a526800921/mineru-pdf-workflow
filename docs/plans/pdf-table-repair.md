...
| 阶段 3 | 真实样本扩展 | 阶段 2 独立验收通过，阶段 3 准入复核通过 | 多个真实样本完成 audit → draft → 确认 → apply → manifest 闭环 | 待实施 |
| 阶段 4 | 独立验收与治理收尾 | 阶段 3 完成 | 回归、真实样本、回滚和治理检查全部通过 | 设计中 |

## 当前阶段

阶段 3：真实样本扩展。当前尚未开始真实样本回填，首轮实施边界为临时副本，不直接覆盖仓库内 canonical 输出包。
...
| 样本矩阵 | demo20 p14–p16、demo60 p47/p48、春风250Sr p85–p93、p132–p133；每项均记录输入基线、执行命令、预期结果和失败判定 |
...
| 当前阻塞项 | 无；无法映射的外部报告页码只登记为待确认，不作为阶段完成证据 |
| 最新独立准入复核 | 2026-07-13，阶段 3，结论通过，见下方独立复核记录和阶段 3 详细准入复核 |
...
| 阶段 | 阶段 3 |
| 结论 | 通过：达到待实施标准 |
| 证据 | 阶段 3 Step 0、样本矩阵、验证方式和失败/回滚边界已冻结；125/125 shell、54/54 table repair、310 pytest；阶段 2 真实 demo60 p47 apply 后 `pdf-check-fixes` 通过 |
| 复核者 | 独立治理复核 |
