...
tests/test_vlm_eval.py        ← 单测（38 用例，零网络）
```
...
- `tests/test_vlm_eval.py`（新建）：38 单测覆盖页分类/渲染/API(mock)/校验/输出
...
- [x] 38 单测通过（零网络 mock）。→ `python3 tests/test_vlm_eval.py` OK。
- [x] `python3 scripts/check_plan_governance.py .` 通过。
...
Step 0 结论：页面基线已完成，P4c 仍保持 `候选`。进入”设计中”前还需完成本地模型/运行时选型，并固定视觉描述 Schema、人工抽样金标准和验收阈值。后续实施将覆盖此结论。

### 验收记录（P4c，2026-07-10）

**结论：P4c 达到”已完成”标准，5 条完成条件全部达标。**

代码新增（已提交）：`scripts/pdf-eval-vlm`（bash wrapper + heredoc）、`scripts/lib/vlm_eval.py`（Python 库）、`tests/test_vlm_eval.py`（38 单测）。

严格验收证据：

- **10 页混合抽样全部通过**：第 1 页（封面）、33（后视图示意图）、47（下电操作表）、58（行车记录仪）、81（仪表菜单）、92（自动下电设置）、105（通讯录）、116（时间设置）、150（发动机保养）、165（制动系统）——每页 `parse_status=ok`、所有字段齐全、`confidence=0.95`。人审确认页面摘要与内容吻合，无幻觉。
...
- **38 单测全绿**：覆盖页分类三分支（image/normal/sparse）、Schema 校验（有效/缺失/类型错误/范围/None/非 dict/子元素缺失）、字段标准化（点号前缀/text→description）、消息构建（含/无 system prompt）、API mock（成功/异常/非 JSON）、段名解析（有效/rerun/DS_Store）、section 索引、JSONL 写入、fitz 渲染。
