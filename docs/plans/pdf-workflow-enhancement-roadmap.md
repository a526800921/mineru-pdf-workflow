...
> 历史说明（2026-07-11）：本路线图中 P2/P3 的 MCP 工具实现、`tools/list` 验收和 MCP 构建命令仅记录当时已完成的历史事实。当前架构已由 [ADR 0002](../adr/0002-cli-only-workflow.md) 收敛为 CLI-only，不再提供或维护 MCP Server；后续能力以 `scripts/` CLI 和 JSON 输出为准。
...
- **P4c**：多模态 VLM 图表理解（[已完成](pdf-evaluation-suite.md#验收记录p4c2026-07-10)）—— 对 `image_or_sparse` 页用 `fitz` 整页渲染 + qwen3-vl-8b（MLX 8bit）→ `data/vlm_eval.jsonl`。10 页均匀抽样全部通过。
