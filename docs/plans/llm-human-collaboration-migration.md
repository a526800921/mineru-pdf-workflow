...
阶段 4：真实 PDF 协作验收。阶段 0、阶段 1、阶段 2 和阶段 3 已完成；阶段 3 已独立验收通过，阶段 4 Step 0 也已通过独立准入复核，达到 `待实施` 标准；阶段 4 已进入实施，独立验收尚未进行。
...
|---|---|
| 准入状态 | 实施中 |
| 准入结论 | 阶段 4 Step 0 已通过并开始真实协作验收；独立验收尚未进行 |
...
| 当前阻塞项 | 无阶段 4 实施阻塞；独立验收尚未进行，阶段 5 不推进 |
| 最新独立准入复核 | 2026-07-15，结论通过：阶段 4 Step 0 达到 `待实施` 标准；阶段 4 已进入实施 |
...
| 阶段 4：真实 PDF 协作验收 | 验证用户只审批、LLM 完成全流程 | 阶段 3 完成 | 春风250Sr 及代表性样本验收记录 | 实施中 |
...
阶段 3 实施已完成，且独立验收已通过；阶段 4 Step 0 已完成并达到 `待实施`，阶段 4 已进入真实协作验收实施。
...
| 证据 | 阶段 3 独立验收已完成；阶段 4 主样本、异常表格、无业务表格、动态 helper 和旧入口兼容矩阵均冻结并可执行；相关 pytest、133/133 修复回归和真实包检查通过 |
| 复核者 | 独立验收复核 |
