...
- 历史上 `pdf2md` 与 `pdf2md-fix` 曾是两个独立 skill；当前能力已迁入 `pdf2md`，阶段 5 正在删除旧兼容入口；
...
- 将 `pdf2md-fix` 的流程能力迁移到 `pdf2md`，关闭兼容窗口并废弃旧入口；不再维护第二个 skill。
...
- 不因入口合并而抹去 `pdf2md-fix` 的历史治理记录；删除旧 skill 只影响触发入口，不影响修复记录、配置、CLI 或真实 PDF 产物。
...
- `skills/pdf2md/SKILL.md` 已承载统一人工协作入口；`pdf2md-fix` 删除前 hash 为 `a1728f44c08ac516c9180606f4ca9c426d19ffc0ffae7e1ad273ac97c3cc72ea`，项目级和用户级兼容文件已删除；项目级和用户级 `pdf2md` skill 当前仍同步。
...
| 历史入口 | `pdf2md-fix` 已废弃；统一使用 `pdf2md` | 不再产生第二套入口和流程 | 旧名称仍被当作当前 skill 触发入口 | skill 文档与会话输出 |
...
阶段 5：治理收尾与兼容策略决策。阶段 0、阶段 1、阶段 2、阶段 3 和阶段 4 已完成；阶段 4 已独立验收通过。阶段 5 尚未完成自己的 Step 0，保持 `设计中`，不因阶段 4 通过自动进入实施。
...
|---|---|
| 准入状态 | 实施中 |
| 准入结论 | 阶段 4 独立验收通过；用户已明确批准废弃 `pdf2md-fix`，阶段 5 正在执行旧入口删除和活动引用迁移 |
| Step 0 | 已冻结 `已废弃` 决策标准、删除前 hash、无活跃消费者核对、活动引用迁移、回滚方式和不改 CLI/PDF/数据库边界 |
| 样本矩阵 | 删除前 skill hash、全仓反向引用、唯一主流程、真实包检查、全量回归和治理检查均有可执行命令 |
| 验证方式 | 删除项目级/用户级兼容 skill，更新 `pdf2md`、ADR 0003、PLAN_MAP 和本计划的当前关系；再执行反向引用、真实包、全量回归和治理检查 |
| 失败/回滚边界 | 不修改 CLI、真实 PDF 产物、配置和数据库边界；删除后若验证失败，恢复删除前两份 skill 和对应治理文档；历史计划和报告不改写为新事实源 |
| 当前阻塞项 | 无；已核对没有活跃消费者，正在完成删除和活动引用迁移 |
| 最新独立准入复核 | 2026-07-15，阶段 5 Step 0 通过；用户批准后进入实施 |
...
| 人工复核顺序、表格异常候选、跨页表格确认 | `skills/pdf2md/SKILL.md`、`pdf2md-fix-manual-workflow` | 入口、职责和协作方式统一在 `pdf2md`；字段契约继续引用专项计划 |
| VLM 使用边界 | `skills/pdf2md/SKILL.md`、ADR 0003 | 在 `pdf2md` 中保留“证据/候选，不是事实”的边界 |
| Markdown/manifest 原子同步、页锚点和幂等 | `skills/pdf2md/SKILL.md`、`pdf-table-repair` | 在 `pdf2md` 中作为人工修复安全门禁 |
| `manual_fixes.jsonl`、`extraction_overrides.json`、`review_overrides.csv` | 计划和现有输出包契约 | 三类文件职责不合并，按用户确认动作分别更新 |
| `pdf2md-fix` 触发入口 | 已删除的历史兼容 skill | 阶段 5 关闭兼容窗口；当前只允许 `pdf2md` 入口 |
...
| 人工修复能力映射 | `skills/pdf2md/SKILL.md` 与 `docs/plans/pdf2md-fix-manual-workflow.md` 反向核对 | 入口、VLM、安全门禁和产物职责均有唯一落点 | 迁移后丢失修复记录、页锚点、manifest 或 VLM 限制 |
...
- 阶段 3 的兼容迁移作为历史记录保留；阶段 5 已在用户批准后删除 `pdf2md-fix` skill，不删除修复脚本、修复记录或专项计划。
...
| 阶段 4：真实 PDF 协作验收 | 验证用户只审批、LLM 完成全流程 | 阶段 3 完成 | 春风250Sr 及代表性样本验收记录 | 已完成 |
| 阶段 5：治理收尾与兼容策略决策 | 独立验收并执行兼容 skill 废弃 | 阶段 4 通过 | 验收报告、PLAN_MAP 更新、`pdf2md-fix` 废弃决策、删除前 hash 与回滚证据 | 已完成 |
...
阶段 3 实施已完成，且独立验收已通过；阶段 4 已完成真实协作验收并通过独立验收；阶段 5 尚未完成自己的 Step 0。
...
- 阶段 4 通过后才进入阶段 5 的兼容策略决策；阶段 5 已按用户批准删除 `pdf2md-fix`，不新增 MCP。
...
#### 阶段 4 实施证据（2026-07-15）

- 已以 `pdf/春风250Sr` 当前最终输出包启动真实协作验收：读取 `review.md`、canonical Markdown、`manifest.json`、`manual_fixes.jsonl`、`table_repair_draft.jsonl`、`extraction_overrides.json`、`review_overrides.csv`、`ingest_ready.csv` 和 `ingest_batch.jsonl`，未要求用户执行脚本。
- 本轮实际协作事实已覆盖 p85-p90 保养跨页表格、p132-p133 同一故障排查表、TOC、纯数字 key 和入库前导出；用户确认内容事实和表格关系，LLM 负责修复记录、配置同步、审核状态和验证。
- 当前包一致性复核：138 个页锚点、120 条 TOC、12 条已应用人工修复、29 个表格候选、11 条表格修复草案；`ingest_ready.csv` 为 182 行，其中 179 条 `ready`、3 条 `skipped`、0 条 `not_ready`、0 个纯数字 key；冲突数据行 0；`ingest_batch.jsonl` 为 179 条。
- 验证结果：`scripts/pdf-check-fixes pdf/春风250Sr` 通过；`python -m pytest -q tests/test_page_quality.py -k 'p4_body_text_no_false_positive or p6_no_table_no_false_positive'` 为 2 passed；`python -m pytest -q tests/test_pdf_run_helper.py` 为 8 passed；`python -m pytest -q tests/test_table_repair.py` 为 44 passed；`bash tests/test-fix-validate.sh` 为 133/133。
- 本轮只读复核没有修改真实 PDF、Markdown、原始 `segments/`、`content_list*.json` 或数据库；阶段 4 的独立验收尚未进行，阶段 5 不推进。

#### 阶段 4 独立验收（2026-07-15，通过）

验收结论：通过。阶段 4 完成条件全部满足；阶段 5 保持 `设计中`，不因阶段 4 通过自动进入实施。

- 真实包结构：canonical Markdown 有 138/138 个逐页锚点，无空页占位；`toc_tree.json` 有 120/120 条目录；`scripts/pdf-check-fixes pdf/春风250Sr` 通过。
- 修复与审核闭环：`manual_fixes.jsonl` 有 12 条 `applied`；`table_repair_draft.jsonl` 有 11 条 `proposed`；`ingest_ready.csv` 有 182 行，其中 179 条 `ready`、3 条 `skipped`、0 条 `not_ready`、0 个纯数字 key；179 条 ready 全部对应 `review_overrides.csv` 的 `approved`，3 条 skipped 全部对应 `rejected`，每条 ready 均有页码范围。
- 下游集合闭环：`ingest_batch.jsonl` 有 179 条，record_id 集合与 ready 集合完全一致；`conflicts.csv` 无数据行；临时副本重跑 `scripts/pdf-extract-data`、`scripts/pdf-prepare-ingest`、`scripts/pdf-export-ingest` 均退出码 0，结果仍为 182 行、179 ready、179 导出、0 冲突；真实包 Markdown/manifest hash 未改变。
- 用户/LLM 边界：本轮验收复核确认用户只提供 PDF 事实、跨页表格关系和候选审批；LLM 负责读取产物、编排 CLI、维护 `manual_fixes.jsonl`/`extraction_overrides.json`/`review_overrides.csv`、执行验证和交付入库前文件；未要求用户执行脚本，未执行数据库导入。
- 回归证据：`python -m pytest -q` 为 312 passed、5 warnings；`bash tests/test-fix-validate.sh` 为 133/133；临时副本数据链路通过；Git 工作区干净。
...
### 阶段 5 Step 0：兼容策略与治理收尾基线冻结（2026-07-15）

状态：已完成，达到 `待实施` 标准。

#### 基线与用户决策

- 当前项目级和用户级 `pdf2md-fix/SKILL.md` 逐字一致，SHA-256 均为 `a1728f44c08ac516c9180606f4ca9c426d19ffc0ffae7e1ad273ac97c3cc72ea`；两份 `pdf2md/SKILL.md` 逐字一致，SHA-256 均为 `bc73810ae72646377a1aba73aa4ae84b687a351c3efe740db4f85e4415061c05`。
- `pdf2md-fix` 在删除前只保留兼容说明，明确 `pdf2md` 是唯一主入口；阶段 3 已完成入口收敛，阶段 4 已验证旧入口不产生第二套结果，阶段 5 已按用户批准删除兼容文件。
- 用户已明确批准 `已废弃`：`rg` 未发现项目或用户 skill 目录之外的活跃消费者；历史计划、报告和提交记录中的名称属于背景/历史记录，不作为当前触发入口。删除项目级和用户级 `pdf2md-fix/SKILL.md`，后续只维护 `pdf2md`。

#### 决策标准

| 最终关系 | 必须满足 | 实施动作 | 回滚 |
|---|---|---|---|
| `已废弃`（已选择） | 旧触发方式无活跃消费者；活动引用已迁移；用户明确批准删除；已记录删除前 hash 和回滚包 | 删除项目级/用户级兼容入口，更新当前 `pdf2md`、ADR 0003、PLAN_MAP 和本计划；历史计划仅保留背景记录 | 恢复删除前两份 skill、活动引用和 hash 记录 |

#### 阶段 5 Step 0 样本/fixture 矩阵

| 场景 | 可执行命令 | 预期结果 | 失败判定 | 输出位置 |
|---|---|---|---|---|
| 删除前 skill 基线 | `git show 24d9725:skills/pdf2md-fix/SKILL.md | shasum -a 256`；`test ! -e skills/pdf2md-fix/SKILL.md && test ! -e /Users/jafish/.claude/skills/pdf2md-fix/SKILL.md` | 删除前基线 hash 为 `a1728f44c08ac516c9180606f4ca9c426d19ffc0ffae7e1ad273ac97c3cc72ea`，两个当前入口均不存在 | 无法恢复删除前内容，或任一兼容 skill 仍存在 | Git 历史、命令输出 |
| 唯一事实源与活动引用 | `rg -n 'pdf2md-fix|pdf2md|兼容入口|已合并|已废弃' docs/PLAN_MAP.md docs/plans/llm-human-collaboration-migration.md docs/adr/0003-llm-orchestrated-dynamic-assistants.md skills/pdf2md/SKILL.md` | 当前治理文档和主 skill 明确旧入口已废弃；不再出现当前兼容入口承诺 | 当前文档仍宣称保留/触发旧入口，或主流程重复定义 | rg 输出 |
| 真实包和历史验收 | `scripts/pdf-check-fixes pdf/春风250Sr && python -m pytest -q && bash tests/test-fix-validate.sh` | 真实包、全量测试和修复回归继续通过 | 任一回归失败或真实包 hash/manifest 不一致 | 测试输出、`pdf/春风250Sr/` |
| 治理一致性 | `plan-governance-cli check . --strict-readiness` | 当前计划无新增治理错误；历史计划缺陷单独记录 | 当前计划出现状态、阶段、证据或反向引用漂移 | 治理检查输出 |

#### 阶段 5 Step 0 完成条件、失败策略与回滚

- `pdf2md-fix` 的最终关系已按用户批准确定为 `已废弃`，删除前 hash 和回滚边界已记录。
- `PLAN_MAP.md`、本计划、ADR 0003、项目级/用户级 skill 和阶段 4 验收证据对“唯一主入口、兼容入口、用户审批、no-MCP、no-database”没有矛盾定义。
- 所有正式引用均能区分历史能力事实源和当前入口事实源；历史计划只保留背景，不重新成为新规范事实源。
- 失败时只恢复兼容 skill 和治理文档，不改 CLI、不改 PDF 产物、不改配置和数据库；阶段 4 已验收产物保持不变。

#### 阶段 5 Step 0 独立准入复核（2026-07-15）

结论：通过，达到 `待实施` 标准；用户批准后阶段 5 进入实施。

证据：阶段 4 已独立验收通过；两份 `pdf2md-fix` skill 的删除前 hash 一致；用户已明确批准废弃；`rg` 未发现活跃消费者；删除前置条件、活动引用迁移、批准边界和回滚方式已明确；反向引用、真实包、全量回归和治理检查命令已登记；无阶段 5 准入阻塞。
...
阶段 5 独立验收通过后，计划状态更新为 `已完成`；`pdf2md-fix` 的最终状态确定为 `已废弃`，验收确认删除和引用迁移没有引入漂移。

#### 阶段 5 独立验收（2026-07-15，通过）

- 用户明确批准废弃 `pdf2md-fix`；项目级和用户级 `pdf2md-fix/SKILL.md` 已删除，删除前基线 hash 为 `a1728f44c08ac516c9180606f4ca9c426d19ffc0ffae7e1ad273ac97c3cc72ea`，可由 Git 提交 `24d9725` 回滚恢复。
- 当前唯一入口为 `pdf2md`；项目级和用户级 `pdf2md/SKILL.md` 仍逐字一致，hash 均为 `b277f601d5c49824fdfb434b6db2fa3599ac27e2051e68358d902c5103395d74`。
- 活动治理文档已迁移到 `pdf2md` 主入口；历史计划、报告和提交记录中的 `pdf2md-fix` 仅保留背景、历史能力或产物字段来源，不再宣称当前兼容入口存在。
- `scripts/pdf-check-fixes pdf/春风250Sr` 通过；`python -m pytest -q` 为 `312 passed, 5 warnings`；`bash tests/test-fix-validate.sh` 为 `133/133`；`git diff --check` 通过。
### Test Coverage

本计划涉及的动态 helper、修复应用、表格候选、回滚/幂等、抽取门禁和真实包回归均纳入 `312 passed` 与 `133/133` 修复契约测试；本次仅删除 skill 文档入口，没有新增未覆盖的代码路径。
- 真实 PDF 包、Markdown、manifest、配置、审核状态、入库前批次和 CLI 契约未被修改；没有数据库写入。严格治理检查仍报告历史已完成计划缺少 Step 0/测试覆盖率证据，但当前迁移计划已无阶段状态或最新复核漂移。

结论：阶段 5 完成，`pdf2md-fix` 已废弃，`pdf2md` 成为唯一用户入口；本迁移计划全阶段完成。
...
- 删除前 `skills/pdf2md-fix/SKILL.md`（已删除，回滚基线见 Git 提交 `24d9725`）
- `/Users/jafish/.claude/skills/pdf2md/SKILL.md`
- 删除前 `/Users/jafish/.claude/skills/pdf2md-fix/SKILL.md`（已删除，内容与项目级删除前基线一致）
- `scripts/`
- `manual_fixes.jsonl`
...
| 风险 | 控制措施 | 回滚 |
|---|---|---|
| 用户需要记忆两个 skill | `pdf2md` 单一主入口，`pdf2md-fix` 已废弃 | 恢复删除前兼容 skill，保留统一流程不变 |
...
- 阶段 1、阶段 2、阶段 3、阶段 4 和阶段 5 已完成；`pdf2md-fix` 已废弃，当前没有迁移阻塞。
- 动态脚本默认只保留在临时目录；需要复现时登记包级副本，并保留 hash、命令和验证结果。
- LLM 每轮交付摘要继续使用事务包装器输出的机器可读 JSON；尚未形成需要独立版本化的外部 Schema。
- 阶段 3 兼容窗口已在阶段 4 独立验收后关闭；用户批准阶段 5 执行 `已废弃`，删除后只保留历史计划和产物字段的背景引用。
...
| 日期 | 2026-07-15 |
| 阶段 | 阶段 5：治理收尾与兼容策略决策 |
| 结论 | 通过：阶段 5 独立验收完成；计划全阶段已完成 |
| 证据 | 用户批准并删除两份 `pdf2md-fix` skill；主 `pdf2md` skill hash/cmp 一致；活动引用迁移、真实包检查、312 pytest、133/133 修复回归和 `git diff --check` 通过；无 PDF 产物、CLI 或数据库变化 |
| 复核者 | 独立验收复核 |
...
| 2026-07-15 | 独立验收复核 | 阶段 4：真实 PDF 协作验收 | 通过：阶段 4 独立验收完成；阶段 5 保持 `设计中` | 真实包 138/138 页锚点、120/120 TOC、179 ready 与 179 导出集合一致；ready 审核状态和页码闭环通过；临时副本抽取/审核准备/导出均为 0 退出码；312 pytest、133/133 修复回归和真实包检查通过；未写数据库 |
| 2026-07-15 | 独立准入复核 | 阶段 5：治理收尾与兼容策略决策 | 通过：阶段 5 Step 0 达到 `待实施` 标准；阶段 5 尚未实施 | 阶段 4 独立验收通过；两组 skill hash/cmp 一致；`已合并`/`已废弃` 标准、反向引用、回滚边界和治理检查矩阵已冻结；无阶段 5 准入阻塞 |
| 2026-07-15 | 独立验收复核 | 阶段 5：治理收尾与兼容策略决策 | 通过：阶段 5 独立验收完成；`pdf2md-fix` 已废弃；计划全阶段完成 | 两份兼容 skill 已删除且删除前 hash 可由 `24d9725` 恢复；两份主 `pdf2md` skill hash/cmp 一致；活动引用迁移；真实包检查、312 pytest、133/133 修复回归、git diff 检查通过；无 PDF、CLI、数据库变化 |
