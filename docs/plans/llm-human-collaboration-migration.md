...
阶段 3：`pdf2md-fix` 兼容迁移。阶段 0、阶段 1 和阶段 2 已完成，阶段 3 Step 0 已完成独立准入复核，达到 `待实施` 标准；本阶段尚未修改两个兼容 skill。
...
|---|---|
| 准入状态 | 待实施 |
| 准入结论 | 阶段 3 Step 0 通过，达到 `待实施` 标准；阶段 3 尚未实施 |
| Step 0 | 已冻结 `pdf2md` 主入口、`pdf2md-fix` 兼容跳转、项目级/用户级 skill 同步、兼容窗口、回滚和不改 CLI/PDF 包边界；两个旧 skill 仍保留完整基线 |
| 样本矩阵 | 两组 skill hash/cmp、`pdf2md` 主章节检索、旧入口兼容文本检索、真实包 `pdf-check-fixes`、312 pytest、133/133 修复回归；均有可执行命令、预期结果、失败判定和输出位置 |
| 验证方式 | 先在提交前保存两个 `pdf2md-fix` skill 的基线 hash；实施后检查兼容说明唯一事实源、两份用户级副本同步、旧触发词仍存在、真实包和全量回归不变，并运行治理检查 |
| 失败/回滚边界 | 阶段 3 只修改项目级/用户级 `pdf2md-fix/SKILL.md` 和治理文档；任何兼容入口、主入口、CLI、PDF 包或数据库边界变化都判失败；回滚为恢复旧 skill hash，不改真实输出包 |
| 当前阻塞项 | 无阶段 3 准入阻塞；兼容入口保留到阶段 4/5 独立验收后再决定 `已合并` 或 `已废弃`，本阶段不删除入口 |
| 最新独立准入复核 | 2026-07-15，结论通过：阶段 3 Step 0 达到 `待实施` 标准；阶段 3 尚未实施 |
...
|---|---|
| 日期 | 2026-07-15 |
| 阶段 | 阶段 3：`pdf2md-fix` 兼容迁移 |
| 结论 | 通过：阶段 3 Step 0 达到 `待实施` 标准；阶段 3 尚未实施 |
| 证据 | 两份 `pdf2md-fix` skill hash/cmp 一致并保留完整旧流程；两份 `pdf2md` skill hash/cmp 一致；统一入口、兼容窗口、回滚边界和唯一事实源已明确；真实包检查、312 pytest、133/133 修复回归可复现 |
| 复核者 | 独立治理复核 |
