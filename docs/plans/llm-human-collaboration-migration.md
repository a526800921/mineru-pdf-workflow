...
阶段 4：真实 PDF 协作验收。阶段 0、阶段 1、阶段 2 和阶段 3 已完成；阶段 3 已独立验收通过。阶段 4 尚未完成自己的 Step 0，保持 `设计中`，不因阶段 3 通过自动进入实施。
...
|---|---|
| 准入状态 | 设计中 |
| 准入结论 | 阶段 3 独立验收通过；阶段 4 尚未完成 Step 0，未进入实施 |
| Step 0 | 阶段 4 的真实 PDF 协作基线尚未冻结；待建立春风250Sr、异常表格、无业务表格页面和旧入口触发的可执行矩阵 |
| 样本矩阵 | 阶段 4 验收场景已列出，但命令、预期、失败判定和输出位置尚未作为 Step 0 独立冻结 |
| 验证方式 | 先完成阶段 4 Step 0 和独立准入复核，再验证用户审批边界、动态 helper 闭环、最终配置一致性和入库前导出 |
| 失败/回滚边界 | 阶段 4 实施前不修改真实 PDF 包或扩大兼容入口范围；准入不足时保持 `设计中`，不推进阶段 4 实施 |
| 当前阻塞项 | 无阶段 3 验收阻塞；阶段 4 需要先完成自己的 Step 0、验证方式、完成条件和独立准入复核 |
| 最新独立准入复核 | 2026-07-15，阶段 3 独立验收通过；该结论只关闭阶段 3，不替代阶段 4 Step 0 |
...
| 阶段 4：真实 PDF 协作验收 | 验证用户只审批、LLM 完成全流程 | 阶段 3 完成 | 春风250Sr 及代表性样本验收记录 | 设计中 |
...
阶段 3 实施已完成，且独立验收已通过；阶段 4 仍需完成自己的 Step 0 和独立准入复核，计划状态保持 `设计中`，不自动推进阶段 4。
...
- 阶段 1、阶段 2 和阶段 3 已完成；阶段 4 尚未完成 Step 0，当前没有实施阻塞，但不具备进入实施的准入证据。
...
| 结论 | 通过：阶段 3 独立验收完成；阶段 4 保持 `设计中`，未自动推进 |
| 证据 | 两份 `pdf2md-fix` skill 和两份 `pdf2md` skill 均 hash/cmp 一致；兼容契约、唯一事实源和回滚边界通过反向检查；真实包检查、312 pytest、133/133 修复回归、GitNexus 变更范围检查通过 |
| 复核者 | 独立验收复核 |
