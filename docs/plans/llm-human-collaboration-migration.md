...
阶段 3：`pdf2md-fix` 兼容迁移。阶段 0、阶段 1 和阶段 2 已完成，阶段 3 Step 0 已完成独立准入复核并开始实施；两个兼容 skill 已收敛，独立验收尚未进行。
...
|---|---|
| 准入状态 | 实施中 |
| 准入结论 | 阶段 3 Step 0 已通过并开始实施；兼容 skill 已收敛，独立验收尚未进行 |
...
| 当前阻塞项 | 无阶段 3 实施阻塞；兼容入口保留到阶段 4/5 独立验收后再决定 `已合并` 或 `已废弃`；阶段 3 独立验收尚未进行 |
| 最新独立准入复核 | 2026-07-15，结论通过：阶段 3 Step 0 达到 `待实施` 标准；阶段 3 已进入实施 |
...
| 阶段 3：`pdf2md-fix` 兼容迁移 | 将人工复核能力迁入 `pdf2md`，保留短期兼容入口 | 阶段 1 和阶段 2 通过 | 项目级/用户级 skill 同步、兼容说明、旧入口回归 | 实施中 |
...
阶段 3 实施已完成，下一步只做独立验收；在独立验收通过前，计划状态保持 `实施中`，阶段 4 不推进。
...
- 阶段 1 和阶段 2 已完成；阶段 3 兼容 skill 已完成实施，正在等待独立验收，无当前实施阻塞。
...
| 证据 | 两份 `pdf2md-fix` skill 已收敛且 hash/cmp 一致；两份 `pdf2md` skill hash/cmp 一致；统一入口、兼容窗口、回滚边界和唯一事实源已明确；真实包检查、312 pytest、133/133 修复回归可复现 |
| 复核者 | 独立治理复核 |
