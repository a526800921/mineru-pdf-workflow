...
阶段 2：动态辅助脚本安全执行层。阶段 0 和阶段 1 已完成，阶段 2 Step 0 已完成准入复核，阶段 2 进入实施；阶段 3 尚未启动。
...
|---|---|
| 准入状态 | 实施中 |
| 准入结论 | 阶段 2 Step 0 通过，进入动态辅助脚本安全执行层实施；阶段 3 尚未启动 |
| Step 0 | 阶段 2 已完成独立 Step 0：现有 CLI 回滚/幂等基线已复核，动态辅助脚本的命令、作用域、备份、dry-run、回滚、留痕和晋升契约已冻结 |
| 样本矩阵 | `pdf/春风250Sr`；`scripts/pdf-check-fixes`；临时副本执行 `pdf-extract-data`、`pdf-prepare-ingest`、`pdf-export-ingest`；预期/实测均为 182 行、179 ready、3 skipped、0 not_ready、0 冲突、0 数字 key |
| 验证方式 | 项目级/用户级 skill hash、真实包只读检查、临时副本端到端复跑、304 pytest、133/133 修复回归、`git diff --check` |
| 失败/回滚边界 | 动态辅助脚本只在临时副本或显式授权的派生产物上执行；dry-run 有任何写入、apply 越界、命令失败或验证失败时整组恢复快照；PDF、原始 `segments/`、`content_list*.json` 永不在授权范围内 |
| 当前阻塞项 | 无阶段 2 实施阻塞；阶段 2 留存策略采用“默认临时目录，需复现时由 LLM 登记包内辅助脚本/配置”；阶段 3 兼容窗口仍未启动 |
| 最新独立准入复核 | 2026-07-15，结论通过：阶段 2 Step 0 达到实施标准，阶段 2 进入实施；阶段 3 尚未启动 |
...
- 阶段 2 完成条件：安全执行包装器、动态命令约定和最小 fixture 均落地；成功、越界、失败回滚、dry-run 保护和幂等用例通过；范围外 hash 不变；临时脚本与包内登记规则写入 `pdf2md` skill；治理检查和独立验收证据完成。
...
本次实施没有修改 PDF、真实输出包、原始 `segments/`、`content_list*.json` 或数据库边界；阶段 2 仍需在完成独立验收记录后关闭。
...
| 阶段 1：统一 `pdf2md` 编排入口 | 把解析后复核、修复、抽取和入库前准备编排到 `pdf2md` | 阶段 0 完成 | `pdf2md` 主流程章节、阶段状态和用户确认模板 | 实施中 |
| 阶段 2：动态辅助脚本安全执行层 | 把一次性脚本纳入备份、dry-run、hash、范围和回滚闭环 | 阶段 0 契约冻结 | helper harness/约定、最小失败回滚 fixture、运行留痕 | 实施中 |
...
- 当前没有业务阻塞；阶段 1 已完成，阶段 2/3 尚未启动。
- 需要在阶段 2 决定动态脚本默认只保留在临时目录，还是对可复现案例保留包级副本；两者都必须保留 hash 和命令。
- 需要在阶段 2 决定 LLM 每轮交付摘要是否固定为机器可读 JSON；如果形成公共契约，需单独记录 Schema。
...
| 日期 | 2026-07-15 |
| 阶段 | 阶段 2：动态辅助脚本安全执行层 |
| 结论 | 通过：阶段 2 Step 0 达到实施标准，阶段 2 进入实施；阶段 3 尚未启动 |
| 证据 | 项目级/用户级 `pdf2md` hash 一致；`pdf-check-fixes` 通过；304 pytest、133/133 修复回归通过；`pdf2md-fix` 保持原 hash；未发生计划外 CLI、PDF 包或数据库边界变化 |
| 复核者 | 独立治理复核 |
