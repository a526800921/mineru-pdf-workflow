...
| 阶段 2 | VLM sidecar 按需覆盖优化 | 阶段 0 证明视觉页收益超过资源成本 | sidecar 调度或报告增强 | 已完成 |
| 阶段 3 | 跨页表格上下文/语义关联 | 阶段 3 自己的 Step 0 证明结构化收益，且独立准入通过 | 受限上下文证据或候选关联产物 | 设计中 |
...
阶段 3：跨页表格上下文 Step 0 评估。阶段 2 已完成独立实施验收；阶段 3 尚未达到 `待实施` 标准，当前只保留候选/证据评估范围。

### 阶段 3 准入摘要

| 字段 | 内容 |
|---|---|
| 准入状态 | 未通过 `待实施`；保持设计中/候选评估 |
| Step 0 | 已用 demo20、demo60、春风250Sr 的真实 canonical、table_candidates、table_repair_draft 和 quick_lookup_draft 复核跨页连续性、错误关联和下游缺口。 |
| 样本矩阵 | S3-1 demo20 p14-p16 明确连续表格；S3-2 demo60 p42-p48 错误连续分组与 p47-p48 不同表头；S3-3 春风250Sr p85-p94 两个保养周期混合、p132-p133 故障表连续；S3-4 三包结构化抽取的多页覆盖与候选关联统计。 |
| 验证方式 | 在临时副本重建 `pdf-table-repair` draft，检查共享 table_id 的精确性；对 canonical 表头、页锚点和 `quick_lookup_draft.csv` 的 page/table 来源做 before 基线；记录可减少的明确缺口及误关联数。 |
| 完成条件 | 只有在明确减少结构化缺口且误关联可控时，才实施只读上下文报告；任何候选均需来源 hash、页锚点、table_id 和人工门禁，不改 canonical。 |
| 失败/回滚边界 | 不能确认连续语义、表头重复、图片表格或页边界时只输出 `needs_review`/不输出关联；删除临时报告即可回滚，不触碰 canonical、chunks、manifest 主状态。 |
| 当前阻塞项 | 当前下游没有多页覆盖行：三包 `quick_lookup_draft.csv` 的 `page_start != page_end` 为 0；现有连续页分组存在明显误报，尚无足够收益证明通用关联值得实施。 |
| 最新独立准入复核 | 2026-07-25：未通过 `待实施`；建议阶段 3 保持候选/只读证据评估，不进入自动关联实现。 |
...
### 阶段 3 Step 0 证据（2026-07-25）

实验只读取真实包，`pdf-table-repair` 运行使用临时副本；没有覆盖仓库内 PDF、canonical Markdown、chunks、manifest 或入库产物。

| 样本 | 现状与证据 | 结论 |
|---|---|---|
| demo20 p14-p16 | canonical 在 p14、p15、p16 分别有表格；p15 延续 p14 的规格表，p16 继续列出“百公里综合油耗/电器装置”等字段。临时副本重建 draft 得到 `demo20_table_p0014-p0016`。 | 明确存在一组跨页语义，但当前页级抽取已保留页面来源，尚未证明必须合并才能减少下游缺口。 |
| demo60 p42-p48 | 现有连续候选规则把 7 页归为 `demo60_table_p0042-p0048`；其中 p47 表头为“下电方式/操作步骤/注意事项”，p48 表头为“解锁方式/操作步骤/注意事项”，不是同一张表。 | 明确误关联失败样本；连续页码不能作为跨页 table_id 的充分条件。 |
| 春风250Sr p85-p94 | 现有规则把 10 页归为 `春风250Sr_table_p0085-p0094`，但 p86-p88 是“磨合期内保养间隔”，p89-p94 是“磨合期后保养间隔”；两组表头语义不同。 | 明确误关联失败样本；重复表头还需要识别周期/章节上下文。 |
| 春风250Sr p132-p133 | 两页表头均为“现象/部位/原因/处理”；p132 的“动力不足”跨到 p133，当前 `quick_lookup_draft.csv` 产生两个同名 key，分别来源于 `html_table:84` 和 `html_table:85`。 | 明确连续语义样本，也是唯一发现的结构化上下文缺口；但应先补父级/续表证据，不应物理合并 canonical。 |

复现命令（只写临时目录）：

```bash
probe=$(mktemp -d)
cp -R pdf/demo20 "$probe/demo20"
cp -R pdf/demo60 "$probe/demo60"
cp -R pdf/春风250Sr "$probe/春风250Sr"
scripts/pdf-table-repair "$probe/demo20"
scripts/pdf-table-repair "$probe/demo60"
scripts/pdf-table-repair "$probe/春风250Sr"
```

复现结果：demo20 生成 p14-p16 共享 ID；demo60 生成 p42-p48 共享 ID；春风250Sr 生成 p85-p94 和 p132-p133 共享 ID。该结果证明当前实现会把“连续候选页”误当作“同一跨页表”，不能直接作为阶段 3 的正向基线。

结构化下游基线：对 demo20、demo60、春风250Sr 和春风 150AURA 的现有 `quick_lookup_draft.csv` 检查，`page_start != page_end` 的行均为 0；春风250Sr p132-p133 的“动力不足”重复行是页级来源分裂，而不是多页行缺失。现有证据因此只支持“候选/证据报告”或针对该类父级续表语义的专项设计，不支持通用自动跨页关联。

### 阶段 3 当前独立结论（2026-07-25）

- 阶段 3 未达到 `待实施` 标准：虽然存在真实连续语义样本，但现有规则的误关联已经在 demo60、春风250Sr 中复现，且没有可量化的多页覆盖收益。
- 暂不修改 `_detect_cross_page_candidates`、`pdf-table-repair` 的公共 draft 契约或 canonical 生成链路；这些修改需要另设精确的表头/章节/续表识别准入，不能以“继续优化”为理由直接放宽。
- 当前推荐：把阶段 3 保持为候选；若后续下游确实需要解决 p132-p133 的父级语义，再另建受限的“续表父级上下文”方案，验收目标限定为重复 key 消歧和 `needs_review` 门禁，不做通用表格合并。
...
| 日期 | 2026-07-25 |
| 阶段 | 阶段 3 |
| 结论 | 未通过 `待实施`：保持候选/只读证据评估，不进入自动跨页关联实现 |
| 证据 | demo20 p14-p16 为明确连续样本；demo60 p42-p48、春风250Sr p85-p94 复现连续候选误关联；四包 `quick_lookup_draft.csv` 多页行数均为 0；春风250Sr p132-p133 仅发现重复 key 的父级上下文缺口。 |
...
- 阶段：阶段 3
- 结论：未通过 `待实施`：保持候选/只读证据评估，不进入自动跨页关联实现
- 证据：临时副本重建证明连续候选规则会把 demo60 p42-p48、春风250Sr p85-p94 错误合并；当前下游多页行数为 0；阶段 3 尚无可量化的通用关联收益
- 复核者：独立治理复核

### 历史复核

阶段 0、阶段 1、阶段 2 和阶段 3 Step 0 的本轮复核已完成；阶段 3 未达到 `待实施`，保持候选。既有专项计划的独立验收不替代后续阶段准入。
...
| 2026-07-24 | 独立验收复核 | 阶段 2 | 通过：阶段 2 实施验收完成并关闭；阶段 3 保持设计中 | 374 pytest、VLM service 7/7、133/133 修复回归、4/4 阶段回归；默认 97 页超限 exit=2；显式 2 页 sidecar 调度通过；严格治理检查和 skill cmp 通过 |
| 2026-07-25 | 独立治理复核 | 阶段 3 Step 0 | 未通过 `待实施`：保持候选/只读证据评估，不进入自动跨页关联实现 | 临时副本 `pdf-table-repair` 复现 demo20 p14-p16、demo60 p42-p48、春风250Sr p85-p94/p132-p133；demo60 与春风250Sr 存在误关联；四包 `quick_lookup_draft.csv` 多页行数为 0；canonical 未改动 |

## 完成证据

- 阶段 2 已实现并提交于 `7005497`：`pdf-eval-vlm` 默认视觉页单批上限为 10 页，支持 `PDF_EVAL_VLM_PAGES` 显式页选择，超限返回非零；sidecar 失败仍保留失败证据，不覆盖 canonical/page node/chunks/manifest 主状态。
- 当前仓库复验：`python -m pytest -q` 为 `374 passed, 5 warnings`；`bash scripts/test-vlm-service.sh` 为 `7/7`；`bash tests/test-fix-validate.sh` 为 `133/133`；`bash test-phase3.sh` 为 `4/4`；默认 97 页超限门禁和显式 2 页调度边界通过。
- `plan-governance-cli check . --strict-readiness`、`git diff --check` 和项目级/用户级 `pdf2md` skill 字节同步检查通过；阶段 3 未实施，继续保持 `设计中`。

## Test Coverage（当前基线）

本轮阶段 1+2 回归：`python -m pytest -q` 为 `374 passed, 5 warnings`；`bash scripts/test-vlm-service.sh` 为 `7/7`；`bash tests/test-fix-validate.sh` 为 `133/133`；`bash test-phase3.sh` 为 `4/4`。另有默认 97 页超限门禁 `exit=2`、显式 2 页调度实测通过、严格治理检查和 skill cmp 通过。既有专项计划中记录的 `312 passed` 属于历史基线；阶段 3 未实施或验收。
