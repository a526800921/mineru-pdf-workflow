...
| 阶段 2 | VLM sidecar 按需覆盖优化 | 阶段 0 证明视觉页收益超过资源成本 | sidecar 调度或报告增强 | 设计中 |
...
阶段 2：VLM sidecar 按需覆盖优化。阶段 1 已独立验收通过；阶段 2 尚未进入实施，必须完成自己的 Step 0、资源收益评估和独立准入复核。
...
|---|---|
| 准入状态 | 设计中 |
| Step 0 | V1/V2 已确认 23 页视觉样本的耗时、客户端/服务端资源参考、Schema/失败门禁、分类一致性和初步人工主要内容一致性；尚需冻结阶段 2 的按需调度范围、人工采纳指标、失败处理矩阵和 sidecar 全流程幂等性。 |
...
| 当前阻塞项 | 需要冻结资源收益门槛、人工采纳指标、失败处理矩阵和 sidecar 全流程幂等性，并完成独立准入复核；不阻塞已完成的阶段 1。 |
| 最新独立准入复核 | 尚未进行；阶段 1 验收记录保留在历史复核中，不替代阶段 2 Step 0。 |
...
- 当前结论：按需 sidecar 有明确证据，但按本次均值全量处理 97 个视觉页约需 22 分钟，且还要承担服务端内存压力；阶段 2 继续保持 `设计中`，不进入默认全量调度。
...
| 日期 | 2026-07-24 |
| 阶段 | 阶段 2 |
| 结论 | 尚未达到 `待实施` 标准；保持 `设计中`，不扩大默认调度 |
| 证据 | V1/V2 共 23 页、21/23 `ok`、客户端峰值约 154–165 MiB、服务端约 10.99 GiB 预算参考、40/40 VLM 门禁测试、固定响应幂等性和 canonical 不变性均已有证据；但资源收益门槛、人工采纳指标、调度上限和真实服务失败处理矩阵仍待冻结。 |
...
- 日期：2026-07-24
- 阶段：阶段 2
- 结论：尚未达到 `待实施` 标准，保持 `设计中`
- 证据：V1/V2 资源、Schema、人工抽查、失败门禁和固定响应幂等性已补齐；仍缺冻结的资源收益门槛、人工采纳指标、调度上限和真实服务失败处理矩阵
- 复核者：独立治理复核

### 历史复核

阶段 0 和阶段 1 的本轮复核已完成；阶段 2 的独立准入复核尚未进行。既有专项计划的独立验收不替代本计划阶段 2 的 Step 0。
...
本轮阶段 1 回归：`python -m pytest -q` 为 `368 passed, 5 warnings`；`bash tests/test-fix-validate.sh` 为 `133/133`；`bash test-phase3.sh` 为 `4/4`。既有专项计划中记录的 `312 passed` 属于历史基线。该基线不代表阶段 2/3 已实施或验收通过。
