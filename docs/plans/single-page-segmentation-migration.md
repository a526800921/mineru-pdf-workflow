...
2. **fallback 参数**：首次解析保持现有参数；命中异常后只重跑该页，并将 `image_analysis` 从 `true` 切换为 `false`。
...
- 用户已确认异常触发信号、`image_analysis=true → false` 的 fallback 参数、结果选择原则和原始/fallback 双版本保存边界。
- demo20 第 12 页已有 API/CLI 差异基线：`image_analysis=false` 时空 `<td></td>` 为 0，复刻 CLI 默认参数时为 16,311。
...
4. 原有 `image_analysis=true → false`、同源产物和用户确认的表格异常阈值已固定。
