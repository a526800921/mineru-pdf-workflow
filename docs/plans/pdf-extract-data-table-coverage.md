...
基线类型：春风250Sr真实输出包的结构化抽取与入库前协作快照。修复前存在重复表头、marker 错位和部分表格漏抽；修复后通过包内配置和用户审批完成抽取、审核和批次导出。本节用于固定当前 PDF 的可观察交付边界，不把未来通用扩展冒充为已完成。

## Test Coverage

当前回归证据：`python -m pytest -q` 为 `312 passed, 5 warnings`；`bash tests/test-fix-validate.sh` 为 `133/133`；真实包 `scripts/pdf-check-fixes pdf/春风250Sr` 通过；入库前导出集合与 `ready` 集合一致。本计划不宣称额外的行覆盖率百分比。

## 计划状态

- 状态：已完成
- 当前阶段：阶段 1：当前 PDF 的 LLM/人工协作闭环（已完成）
- 最后更新：2026-07-15
...
本计划当前交付的是“LLM 编排、人工审批、脚本执行”的入库前协作闭环，不继续作为通用业务语义自动推断项目推进。

- 对 `pdf/春风250Sr`：阶段 1 的网格展开、包内 `extraction_overrides.json`、候选抽取、人工审核、`ready` 门禁和入库前批次已经完成并验收。
- `scripts/pdf-extract-data` 只保留通用网格展开、来源证据、候选生成和状态计算；具体 PDF 的列语义由 LLM 根据用户确认写入包内配置。
- 用户只确认 PDF 事实、跨页关系、列语义和候选状态；LLM 负责读取产物、编排命令、备份/回滚、更新配置和审核文件、执行验证并交付摘要。
- 原阶段 2–4 改为后续候选范围：只有新的真实 PDF 出现可复现漏抽样本时才恢复推进，不阻塞当前 `pdf2md` 主流程。
...
本计划的实施边界补充：具体 PDF 的列语义不写入 `scripts/pdf-extract-data`。脚本只提供通用 HTML 网格展开、页码定位和证据保留；本次会话由 LLM 根据 PDF/Markdown 复核结果生成 `<package>/data/extraction_overrides.json`，把表格 ID 映射到 `header_rows`、`key_column`、`marker_column`、`value_columns` 和分类行规则。该配置随输出包留痕，可由后续人工继续修订。

Step 0 必须把外部报告中的 250Sr 页码映射到当前项目真实输出包，并建立“Markdown 表格总数 → 可抽取表格 → 已生成草案行 → needs_review/冲突 → ready”的覆盖矩阵。外部报告的行数不能直接作为当前仓库完成证据。

### Step 0 补充证据（2026-07-14）

当前 `pdf/春风250Sr` 输出包已完成首轮真实复核：`quick_lookup_draft.csv` 共 61 行，其中 58 行为业务草案、3 行为 p138 页脚联系方式；`pdf-prepare-ingest` 在联系方式拒绝后输出 `not_ready=58`、`skipped=3`、`conflicts=0`。进一步按 `key_role` 抽样发现，保养表草案中存在 9 条重复表头 `小时 → 月份 / km / 备注`，以及 42 条以 `■/▲/-` 为 `key`、实际项目名被拼入 `value` 的记录。

该基线证明当前缺口不是 Markdown 表格内容缺失，而是 `pdf-extract-data` 对 p86–p94 双层表头、`rowspan/colspan` 和首列维修标记的网格归一化不足。p86–p94 canonical Markdown 已按 PDF 修复并通过 `pdf-check-fixes`，可作为阶段 1 的输入基线。
...
阶段 1：当前 PDF 的 LLM/人工协作闭环已完成。已完成通用 `rowspan/colspan` 网格展开、包内配置读取、页码范围收窄和冲突 `row_index` 身份补全；完整 canonical 重建后先生成 211 条候选，依据当前包级 `policies.numeric_key=skip` 过滤 29 条图示编号，最终保留 182 条，其中 179 条已由用户确认并进入 ready，3 条页脚联系方式保持拒绝。后续通用覆盖扩展不作为当前计划的未完成项。
...
|---|---|
| 准入状态 | 已完成 |
| Step 0 | 2026-07-14 春风250Sr 真实包基线：修复前 61 行且存在重复表头/marker 错位；修复后 78 行，保养表项目列、marker、间隔列可由包内 JSON 配置复现 |
| 样本矩阵 | `pdf/春风250Sr` 全量 138 页，重点 p85–p94、p132–p133、p138 及图示编号页；命令为 `scripts/pdf-extract-data`、`scripts/pdf-prepare-ingest`、`scripts/pdf-export-ingest`；预期为 182 行、179 ready、0 not_ready、3 skipped、0 冲突，且无纯数字 key |
| 验证方式 | `pytest -q -p no:cacheprovider`、`bash tests/test-fix-validate.sh`、`pdf-check-fixes`、批次 manifest 计数和 record_id 集合一致性校验 |
| 失败/回滚边界 | 配置错误、来源证据缺失、冲突或审核覆盖误配时保持 `not_ready`，删除 `review_overrides.csv` 中对应批准行即可回退，不写数据库 |
| 当前阻塞项 | 无 |
| 最新独立准入复核 | 2026-07-15，阶段 1：当前 PDF 的 LLM/人工协作闭环，结论通过并关闭当前计划，见下方记录 |
...
|---|---|
| 日期 | 2026-07-15 |
| 阶段 | 阶段 1：当前 PDF 的 LLM/人工协作闭环 |
| 结论 | 通过：当前计划完成；后续通用扩展仅按真实样本触发 |
| 证据 | 真实包 182 行，179 ready、0 not_ready、3 skipped、0 conflicts，纯数字 key 为 0；用户确认事实和候选状态，LLM 维护配置/审核文件并完成验证；仅完成入库前准备，未写数据库 |
| 复核者 | 独立治理复核 |
...
| 日期 | 复核者 | 阶段 | 结论 | 证据 |
|---|---|---|---|---|
| 2026-07-14 | 独立治理复核 | 阶段 1：通用网格展开与 LLM/配置驱动列语义 | 通过：达到当前阶段实施标准 | 真实包抽取、审核、导出和回归证据 |
| 2026-07-15 | 独立验收复核 | 阶段 1：当前 PDF 的 LLM/人工协作闭环 | 通过：当前计划完成；后续通用扩展不阻塞主流程 | 312 pytest、133/133 修复回归、真实包检查通过；179 条 ready 与 179 条导出集合一致；用户只确认 PDF 事实和候选状态，未执行数据库导入 |

## 阶段路线图

| 阶段 | 目标 | 进入条件 | 验证方向 | 状态 |
|---|---|---|---|---|
| 阶段 0 | 漏抽覆盖矩阵与审核边界冻结 | 真实包基线可复现 | 覆盖矩阵、失败 fixture、审核门禁 | 已完成 |
| 阶段 1：当前 PDF 的 LLM/人工协作闭环 | 展开复杂表头、配置语义、人工审核和入库前导出 | 阶段 0 证据通过 | 抽取、审核、冲突和导出回归 | 已完成 |
| 阶段 2 | 人工审核与入库门禁通用化 | 新真实样本证明当前契约不足 | ready 门禁和 record_id 兼容 | 候选 |
| 阶段 3 | 真实样本覆盖回填 | 新真实漏抽样本登记 | 高价值表格覆盖率和人工审核统计 | 候选 |
| 阶段 4 | 独立验收 | 后续候选实际启动 | 全量回归、治理和 drift 检查 | 候选 |
...
1. 先实现通用 `rowspan/colspan` 表头网格展开，禁止用第一行直接代表复杂表头；具体表格的项目列和间隔列由 LLM/人工配置传入。
...
本次春风样本的最小实现边界：不改变 `quick_lookup_draft.csv` 的原有字段和 `record_id` 计算字段；由 `extraction_overrides.json` 指定保养表的实际项目列、标记列、间隔列和分类行规则；脚本按配置将小时/月份/km/备注拼接为 `value`，将 `■/▲` 写入 `notes` 并保留在 `evidence_text`；表头行和纯分类行不生成业务草案。冲突 identity 追加 `row_index`，使同一项目的多个合法保养间隔行不被误报为冲突。未能稳定展开的行继续输出 `needs_review`，不自动批准。
...
- 用户只确认 PDF 事实、跨页关系、列语义和候选状态，没有要求用户运行脚本或编辑配置文件。
- LLM 负责读取产物、生成/更新 `extraction_overrides.json`、维护 `review_overrides.csv`、执行抽取/审核/导出和验证。
- 当前包最终为 182 条抽取记录，179 条 `ready`、3 条 `skipped`、0 条 `not_ready`、0 冲突、0 纯数字 key；`ingest_batch.jsonl` 的 179 条 `record_id` 与 ready 集合一致。
- `scripts/pdf-check-fixes pdf/春风250Sr`、`python -m pytest -q`（312 passed）和 `bash tests/test-fix-validate.sh`（133/133）通过；未执行数据库导入。

## 独立验收（2026-07-15）

结论：**通过，当前 PDF 协作闭环完成。** 后续通用覆盖扩展保留为候选，只有新的真实漏抽样本出现时才重新建立 Step 0 和独立准入复核。
