...
阶段 1：通用网格展开与 LLM/配置驱动列语义。当前已完成通用 `rowspan/colspan` 网格展开、包内配置读取、页码范围收窄和冲突 `row_index` 身份补全；完整 canonical 重建后先生成 211 条候选，依据当前包级 `policies.numeric_key=skip` 过滤 29 条图示编号，最终保留 182 条，其中 179 条已由用户确认并进入 ready，3 条页脚联系方式保持拒绝。
...
| 当前阻塞项 | 无业务阻塞；脚本不自动推断具体 PDF 的列语义，纯数字 key 由包级配置过滤 |
| 最新独立准入复核 | 2026-07-14，阶段 1：通用网格展开与 LLM/配置驱动列语义，结论通过，见下方记录 |
...
|---|---|
| 日期 | 2026-07-14 |
| 阶段 | 阶段 1：通用网格展开与 LLM/配置驱动列语义 |
| 结论 | 通过：达到当前阶段实施标准 |
| 证据 | 全量 304 个 pytest、133/133 shell；真实包 182 行，179 ready、0 not_ready、3 skipped、0 conflicts，纯数字 key 为 0；仅完成入库前准备，未写数据库 |
| 复核者 | 独立治理复核 |
...
| 阶段 1：通用网格展开与 LLM/配置驱动列语义 | 展开复杂表头并保留配置/证据 | 阶段 0 证据通过 | 抽取、审核、冲突和导出回归 | 实施中 |
| 阶段 2 | 人工审核与入库门禁兼容 | 阶段 1 候选契约稳定 | ready 门禁和 record_id 兼容 | 待实施 |
| 阶段 3 | 真实样本覆盖回填 | 阶段 2 完成 | 高价值表格覆盖率和人工审核统计 | 待实施 |
| 阶段 4 | 独立验收 | 阶段 3 完成 | 全量回归、治理和 drift 检查 | 设计中 |
