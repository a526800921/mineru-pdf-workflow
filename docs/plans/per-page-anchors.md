...
## 阶段 4 收尾：回归基线闭环比对（2026-07-10）

阶段 4 已实施完成。实施前固化的回归基线用于最终比对：

- `data/quick_lookup_draft.csv` 共 390 条记录，表头保持现有 17 列。
- 实施前基线：326 条单页范围、64 条跨页范围、0 条空页范围，页码范围为 2–191。
- 基线文件 SHA-256：`c7140903cb19fdcf9338c5307f25646701eca83f7f065f7678583c4fe5481ce0`。
- 实施后对比按”阶段 4 验收口径”执行：`page_start`、`page_end` 及由页码驱动的 `section_path` 允许变化；其余字段和状态语义无回归。

闭环比对结果：

- 春风 150AURA：390 行全部单页（跨页 64→0），非页码字段无变化，0 超段。
- demo20/demo60/demo5：全部通过，行数稳定。
- 幂等性：连续两次运行一致。
- M-5 跨段锚点泄漏已修复并回归验证。
- 验收状态：代码实现通过，计划状态统一为已完成。
...
- `extract-data` 逐页收窄属行为迁移 → 需现状快照做回归基线（阶段 4 收尾回归基线）。
