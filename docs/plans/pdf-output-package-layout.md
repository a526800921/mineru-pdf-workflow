...
同一 PDF 连续运行两次，满足以下条件视为通过：

- `images/` 下文件集合和数量一致。
- 不出现重跑叠加式文件名（如 `image_1_1.png`、`image_1_1_1.png`）。
- 合并 Markdown 图片引用集合一致，且引用均指向本地存在的图片文件。

demo5 样本当前为无图片 PDF（`images: 0, refs: 0`），连续运行两次无漂移。demo20 含图片样本补验（2026-07-01）：`pdf-seg` 生成输出包后，连续两次 `pdf-auto` 运行，`images: 13→13`、`refs: 3→3`，无缺失引用、无叠加式文件名。
...
- `pdf-merge` 图片同名不同内容冲突已修复：SHA-256 内容校验，出现冲突时失败并输出冲突路径（`modelpad-pdf-service-lifecycle` 阶段 3）。
...
| 问题 | 推荐方案 | 是否阻塞当前阶段 | 状态 |
|---|---|---|---|
| `data/` 下草案文件由谁生成 | [输出包结构化数据抽取计划](structured-data-extraction.md) 负责 | 否 | 设计中 |
...
| `pdf-merge` 图片同名冲突可能被静默跳过 | `modelpad-pdf-service-lifecycle` 阶段 3 已修复：SHA-256 校验，同名同内容跳过、不同内容失败并输出冲突路径 | 否 | 已解决 |
