...
- 当前 `pdf-merge` 收集图片时按文件名去重，目标文件已存在则跳过；如果不同分段产生同名但内容不同的图片，可能静默保留第一张并让后续引用指向错误资源。后续应增加内容哈希校验、冲突重命名或失败提示。
...
| `pdf-merge` 图片同名冲突可能被静默跳过 | 后续在图片收集阶段增加内容哈希校验、冲突重命名或失败提示；当前 demo5/demo20 幂等样本未暴露该问题 | 否 | 已记录 |
