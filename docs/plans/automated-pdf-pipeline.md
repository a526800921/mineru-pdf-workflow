...
| 阶段 3 | 自动重跑 | JSON 报告可用 | 可疑段 high 重跑并再验证 | 候选 |
| 阶段 4 | 合并与人工兜底 | 重跑策略稳定 | 生成 merged/review 文件 | 候选 |
| 阶段 5 | MCP 接入 | CLI 契约稳定 | Claude Code 可通过 MCP 调用 | 设计中 |

## 当前阶段

阶段 2 已完成，下一阶段为阶段 3（自动重跑）。

### 阶段 2 完成证据

- `PDF_VALIDATE_JSON=1` 在 191 页说明书样本上输出有效 JSON。
- 可疑段（p0000-0019，覆盖率 0.77）与人类可读输出一致。
- `mcp/README.md` 的 `validate_segments` 契约字段（name、start_page、end_page、coverage、status、missing_tokens、markdown_path）与脚本输出一致。
- 计划治理检查通过。
- JSON 在 stdout，人类可读输出在 stderr，默认行为不变。
...
| 可疑段重跑覆盖原目录还是写入 `rerun-high/` | 阶段 3 前做一次样本实验 | 否 | 待确认 |
