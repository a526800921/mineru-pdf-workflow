...
def test_v2_uses_canonical_table_and_raw_row_ids_without_remapping(tmp_path: Path) -> None:
    package = tmp_path / "package"
    data = package / "data"
    data.mkdir(parents=True)
    (package / "fixture.md").write_text(
    ...
    (package / "manifest.json").write_text(
        json.dumps({
            "files": {"markdown": "fixture.md", "pdf": "fixture.pdf"},
    ...
    write_csv(data / "quick_lookup_draft.csv", fields, [{
        ...
        "key": "点火控制方式", "value": "ECU 点火", "unit": "", "page_start": "1",
        "page_end": "1", "evidence_text": "", "confidence": "medium", "status": "draft",
        "notes": "", "source_block_id": "html_table:2", "table_id": "html_table:2",
        "row_index": "2", "parent_key": "", "key_role": "business_key",
    }])
    (data / "extraction_overrides.json").write_text(
        json.dumps({"tables": {"html_table:2": {
            "header_rows": 1, "key_column": 0, "value_columns": {"值": 1},
        }}}, ensure_ascii=False), encoding="utf-8",
    )
